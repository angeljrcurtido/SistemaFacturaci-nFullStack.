...
const Dashboardtotal: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        const [ventas30DiasResponse, totalCantidadResponse] = await Promise.all([
          axios.get('http://localhost:3001/ventas/total-ultimos-30-dias-combinado'),
          axios.get('http://localhost:3001/ventas/total-cantidad-combinado')
        ]);

        const ventas30DiasData = ventas30DiasResponse.data;
        const totalCantidadData = totalCantidadResponse.data;

        setTotalVentas30(ventas30DiasData.totalVentas30Gral);
        setCantidadTotalVentas30(ventas30DiasData.cantidadTotalVentasGral);
        setTotalGanancias30(ventas30DiasData.totalGanancias30Gral);
        setVentasDiarias(Object.values(ventas30DiasData.ventasDiarias));
        setDias(Object.keys(ventas30DiasData.ventasDiarias));
        setTotalCantidadVentas(totalCantidadData.totalCantidadVentasGral);
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4 border-b">Informes Generales Ventas Gral (Contado + Credito)</h1>
